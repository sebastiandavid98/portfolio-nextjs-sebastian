          ...
          <div className="animate-fade-up delay-200 bg-gray-50 dark:bg-slate-800/40 border border-gray-200 dark:border-slate-700/50 rounded-2xl p-7 shadow-sm">
            {status === "sent" ? (
              <div className="animate-scale-in flex flex-col items-center justify-center py-10 text-center gap-4">
                ...
                <button onClick={() => setStatus("idle")} className="mt-1 text-blue-600 dark:text-blue-400 hover:text-blue-500 text-sm font-medium transition-colors underline underline-offset-2">
                  {t.contact.another}
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                {[
                  ...
                  { id: "email", label: t.contact.email, type: "email", placeholder: "tu@email.com" },
                ].map((field) => (
                  <div key={field.id}>
                    <label htmlFor={field.id} className="block text-gray-700 dark:text-slate-300 text-sm font-medium mb-1.5">{field.label}</label>
                    <input id={field.id} name={field.id} type={field.type} required placeholder={field.placeholder}
                      suppressHydrationWarning={field.id === "email"}
                ...
                <div>
                  ...
                  <textarea id="message" name="message" required rows={4} placeholder={t.contact.messagePlaceholder}
