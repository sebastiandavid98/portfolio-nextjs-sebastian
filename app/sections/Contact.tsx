...
export default function Contact() {
  ...
  return (
    <section id="contact" className="relative bg-white dark:bg-slate-950 py-10 px-6 overflow-hidden">
      <div className="absolute top-0 right-0 w-80 h-80 bg-green-50 dark:bg-green-600/5 rounded-full blur-3xl pointer-events-none" />

      <div className="relative max-w-6xl mx-auto">
        <div className="animate-slide-right flex items-center gap-3 mb-3">
          <span className="h-px w-8 bg-green-700" />
          <span className="text-green-700 dark:text-green-400 text-sm font-semibold tracking-widest uppercase">{t.contact.label}</span>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className="animate-fade-up">
            <h2 className="text-4xl sm:text-5xl font-extrabold text-gray-900 dark:text-white leading-tight mb-4">
              {t.contact.title}{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-700 to-green-500">{t.contact.titleAccent}</span>
            </h2>
            ...
            <div className="flex flex-wrap gap-2.5">
              {socials.map((s, i) => (
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label}
                  ...
                  className="animate-fade-up flex items-center gap-2 text-gray-700 dark:text-slate-300 hover:text-green-700 dark:hover:text-green-400 bg-gray-50 dark:bg-slate-800/50 hover:bg-green-50 dark:hover:bg-green-600/10 border border-gray-200 dark:border-slate-700/50 hover:border-blue-300 dark:hover:border-blue-500/40 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 hover:-translate-y-0.5 shadow-sm hover:shadow-md hover:shadow-green-600/10">
                  {s.icon}{s.label}
                </a>
              ))}
            </div>
            ...
            {status === "success" ? (
              <div className="animate-scale-in flex flex-col items-center justify-center py-10 text-center gap-4">
                ...
                <button onClick={() => setStatus("idle")}
                  className="mt-1 text-green-700 dark:text-green-400 hover:text-blue-500 text-sm font-medium transition-colors underline underline-offset-2">
                  {t.contact.another}
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-slate-900/60 border border-gray-300 dark:border-slate-700 focus:border-green-600 focus:ring-2 focus:ring-green-600/20 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 rounded-xl px-4 py-2.5 text-sm outline-none transition-all duration-200"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-green-700 hover:bg-green-600 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold px-6 py-3 rounded-xl transition-all duration-300 shadow-lg shadow-green-600/20 hover:shadow-green-600/30 hover:-translate-y-0.5"
                >
                  {status === "sending" ? (
